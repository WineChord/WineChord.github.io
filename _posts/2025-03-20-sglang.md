...
SGLang （[https://github.com/sgl-project/sglang](https://github.com/sgl-project/sglang)）源于 *SGLang: Efficient Execution of Structured Language Model Programs （[https://arxiv.org/abs/2312.07104](https://arxiv.org/abs/2312.07104)）*这篇 paper，代码应该是 2023 年 10 月份开始开发的，paper 则是 2023 年 12 月份放出来的。
...
![](https://pic3.zhimg.com/v2-59d02f4e716d9f99362401409138414c_r.jpg)
...
![](https://pica.zhimg.com/v2-ddc6f67404ad202b313bf63344227514_r.jpg)




上面这个图里实际上还包含了 LRU（Least Recently Used）驱逐机制，也就是当一块 KV cache 不再被任何地方引用的时候，并且显存告急，就将最近最少使用的叶子结点对应的 KV cache 给驱逐掉。这里的驱逐就是指的完全从 GPU vRAM 里删掉，而不是 offload 到 CPU 上。假如之后又确实需要这部分 KV cache 怎么办？重新计算一遍。
...
1.  原 paper 应该是少了一个 current_size += req.size()
...
前面不还有个定理吗，就是说在 radix tree 上走 DFS（depth-first search）的时候能达到最优缓存命中率，不妨来看看是怎么证明的： 首先令 $R$ 为批次中的请求，$T$ 为从 $R$ 构造出的 radix tree，对于 $T$ 的每一条边 $e$来说，和他相关的 KV cache 至少要计算一次，令 $\left|e\right|$ 为 $e$ 对应的 KV cache 的大小，令 $C$ 表示计算 $R$ 的 KV cache 所需要的计算复杂度，那么我们可以得到一个下界：$C\ge\sum_{e\in\text{edges}(T)}\left|e\right|$，然后考虑对这个 radix tree $T$ 做 DFS，当我们第一次访问并计算 $e$ 的 KV cache 之后，我们会在接下来持续访问 $e$ 的子树，在计算 $e$ 的子树所对应的 KV cache 时，$e$ 所对应的 KV cache 会不断地命中，从而不会产生额外的计算开销，当 $e$ 的子树计算完毕之后，$e$ 本身不会再被用到，因此他对应的 KV cache 可以被放心驱逐，并且此时我们可以注意到，当 cache size $\ge$ 最长请求长度时，在计算 $e$ 的子树 KV cache 时，$e$ 本身一定不会被清除，因此每条边 $e$ 对应的 KV cache 实际上只会被计算一次，于是我们可以达到这个下界 $C=\sum_{e\in\text{edges}(T)}\left|e\right|$ ，而缓存命中率同时达到其上界： $\begin{align} \frac{\sum_{r\in R}r 中命中缓存的\text{ token }数}{\sum_{r\in R} r 中所有的\text{ token }数}=1-\frac{C}{\sum_{r\in R} r 中所有的\text{ token }数} \end{align}$ 
...
![](https://pic1.zhimg.com/v2-106776f5f4bcb9550e2402c603373b42_r.jpg)
...
此处值得额外再展开细细梳理，可以参考阅读 [https://lmsys.org/blog/2024-02-05-compressed-fsm/](https://lmsys.org/blog/2024-02-05-compressed-fsm/)
