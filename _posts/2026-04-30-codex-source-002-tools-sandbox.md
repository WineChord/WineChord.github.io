...
这篇不要从某个具体工具开始读，而要先把“工具调用”拆成三层。

第一层是模型可见层：模型只看到 `ToolSpec` 和参数 schema。第二层是 core 运行时层：router、runtime、registry、approval、sandbox 决定这个调用能否执行、如何排队、是否要问人。第三层才是 handler：shell、`apply_patch`、MCP、Apps、dynamic tools 各自完成具体动作。

这样读的好处是，不会把“模型会调用工具”误解成“模型能直接执行命令”。在 Codex 里，模型只提出意图；执行权始终留在 core 的策略链上。
...
如果要判断一个 Codex 工具路径是否安全、可扩展，可以按下面几项检查：

| 检查点 | 判断方式 | 失效时的风险 |
| --- | --- | --- |
| spec 和 handler 是否分离 | 模型只看 schema，core 保存 handler | 工具实现细节泄漏到 prompt，后续扩展困难。 |
| mutating 工具是否串行 | `is_mutating()` 和 `tool_call_gate` 生效 | 并行写文件、跑命令时互相踩状态。 |
| 审批和 sandbox 是否集中 | 走 `ToolOrchestrator` / `sandboxing.rs` | 每个工具各写一套权限判断，策略漂移。 |
| 文件编辑是否结构化 | `apply_patch` 是一等工具 | diff、审批、trace 都只能从 shell 文本里猜。 |
| 外部工具是否可还原身份 | MCP payload 保留 server/tool/raw args | 模型可见名称和真实工具名对不上。 |
