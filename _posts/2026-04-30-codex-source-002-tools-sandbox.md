...
> 模型只发出工具调用意图；是否能执行、如何执行、以什么权限执行，由 core 工具运行时分层决定。
...
*图 1. 模型只看到工具 schema；core 侧才知道 handler、审批、沙箱、并发和 hooks。读源码时可以先沿着 `ToolRouter::build_tool_call` 到 `ToolRegistry::dispatch_any` 这条线走。*

## 一、先区分 spec、call、handler

### 1.1 模型看到的是 `ToolSpec`

工具在 `build_prompt` 时注入模型请求。上一篇提到，`Prompt.tools` 来自 [`router.model_visible_specs()`][build-prompt]，不是直接来自某个 handler。
...
这个转换层的价值是稳定后续执行链。模型 API 可以用多种方式表达工具调用，但 registry、runtime、approval 和 handler 不应该到处判断 `ResponseItem` 类型。后面统一处理 `ToolCall { tool_name, call_id, payload }` 就够了。

MCP payload 也说明了为什么 router 不能省。MCP 工具的 model-visible name 可能经过清洗、去重或 hash，以适配 API 命名限制；执行时却必须知道原始 server/tool。Codex 在 router 阶段把它还原为 `server + raw tool name + raw arguments`，后面的 MCP handler 才能准确调用对应 server。
...
*图 2. 模型给的是“我要调用某个工具”的意图；系统要在每一道门上重新确认“这个意图能不能以这种方式执行”。*
...
这里追求的不是极致吞吐，而是可解释的状态边界：读可以并发，写要保守串行。
...
### 2.3 Approval 和 sandbox 集中在 orchestrator
...
### 3.1 Shell / Unified Exec：终端执行不是 `Command::new` 的包装
...
### 3.2 `apply_patch` 是一等工具，不是 shell 字符串
...
这张表也是后续读 Codex 工具代码的最小 checklist：先找 spec，再找 router，再看 runtime/registry gate，最后才看具体 handler。
