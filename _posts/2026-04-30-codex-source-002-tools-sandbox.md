...
![Codex 源码剖析系列封面：工具调用、审批、沙箱和模型循环构成一套受控执行系统](/assets/images/posts/2026-04-30-codex-source/series-hero.png)

*图 0：工具系统是 coding agent 最容易失控的地方，因为模型输出会影响真实文件、进程和网络。*

上一篇从 TUI 走到了 `run_turn`。这一篇看更核心的问题：模型输出一个工具调用之后，Codex 如何决定它到底是什么工具、能不能执行、能不能并行、要不要审批、是否应该进入 sandbox、最后如何把结果喂回模型。

源码版本仍然是 [`openai/codex@ac4332c05b11e00ae775a24cb762edc05c5b5932`](https://github.com/openai/codex/tree/ac4332c05b11e00ae775a24cb762edc05c5b5932)。
...
*图 2：这张图把工具执行中几个关键 gate 单独拿出来。模型给的是“我要调用某个工具”的意图，系统要在每一道门上重新确认“这个意图能不能以这种方式执行”。*
...
工具集合的构造在 [`build_specs_with_discoverable_tools`][build-specs]。这里会把内置工具、MCP 工具、deferred MCP tools、dynamic tools、discoverable tools、多 Agent 工具、`tool_search`、`view_image`、`apply_patch`、`unified_exec` 等统一规划成两类东西：
...
Codex 里现在有 shell 相关的多条路径。传统 shell handler 把参数转成 `ExecParams`，其中包含 command、cwd、env、network、sandbox permissions、Windows sandbox level 等，见 [`ShellHandler::to_exec_params`][shell-to-exec-params]。

`UnifiedExecHandler` 则面向更完整的终端交互场景：`exec_command` 可以启动一个命令会话，`write_stdin` 可以向已有 session 写入输入。参数结构包括 `cmd`、`workdir`、`shell`、`login`、`tty`、`yield_time_ms`、`max_output_tokens`、`sandbox_permissions`、`additional_permissions`、`justification`、`prefix_rule`，见 [`ExecCommandArgs`][unified-exec-args]。
