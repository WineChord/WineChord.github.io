...
locale: zh-CN
canonical_url: "https://www.wineandchord.com/llm/agent/codex-source-001-turn-loop/"
categories:
  - LLM
  - Agent
tags:
...
mathjax: true
header:
  og_image: https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-turn-loop-cover.png
  teaser: https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-turn-loop-cover.png
  teaser_alt: "Codex 一次 turn 的执行闭环：TUI、App Server、Session、run_turn 和 EventMsg"
---
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-turn-loop-cover.png" alt="Codex 一次 turn 的执行闭环：TUI、App Server、Session、run_turn 和 EventMsg 串成可恢复 agent runtime">
  <figcaption>图 1. 封面只保留本文要追踪的关键边界：TUI、App Server、core、模型流和事件回流。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-app-server-contract.png" alt="Codex App Server contract：TUI、remote client 和 app-server command 进入 App Server，再以 thread、turn、request、notification、server request 和 overrides 语义连接 core Op queue">
  <figcaption>图 2. App Server contract 图只保留 client surface、protocol boundary、agent harness 和 event stream 的关键关系。</figcaption>
</figure>
...
这也是本文先画 App Server contract、再追 CLI/TUI 的原因。App Server 定义的是跨 client 的协议词；CLI/TUI 定义的是本地用户怎样进入这套协议。两个边界相邻，但不能合并：如果从 TUI widget 直接跳到模型循环，就会把输入框状态、thread selection、turn settings 和 provider request 混成同一层。

下面的 CLI 图只回答“native binary 如何到达 TUI surface”，不回答“模型如何采样”。这个切分能让后面的 `AppCommand::user_turn`、`turn/start` 和 `run_turn` 各自落在正确 owner 上。
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-cli-tui-boundary.png" alt="Codex CLI 到 TUI 的边界：codex.js 选择 native binary，Rust MultitoolCli 分发到 TUI、exec、app-server 和 sandbox，TUI 再构造 AppCommand::user_turn">
  <figcaption>图 3. CLI/TUI 边界图只画 native 分发物、入口分发和 TUI surface；模型循环还没有开始。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-main-loop.png" alt="Codex 主路径：TUI client 的 UserInput 经过 App Server turn/start、core Op queue、RegularTask、Model stream，再以 EventMsg 回到界面">
  <figcaption>图 4. 主路径图只保留从 TUI 到模型流、再从事件回到 UI 的关键节点。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-turn-terms.png" alt="Codex 五个术语跨三层边界：Thread、Turn、Submission/Op、ResponseItem 和 EventMsg 分别属于 UI、core 与 model view 的不同切面">
  <figcaption>图 5. 术语图只保留 Thread、Turn、Submission/Op、ResponseItem 和 EventMsg 的边界关系。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v3-input-structure.png" alt="UserInput 结构：用户意图与 turn settings、thread state 合并为 core Op::UserInputWithTurnContext，再经 build_prompt 投影成 model-visible input">
  <figcaption>图 6. 输入结构图用少量标签标出 UI intent、turn settings、UserInputWithTurnContext、build_prompt 和 model-visible input 的分层。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-turn-start-boundary.png" alt="turn/start 边界：load_thread、map v2 input、apply overrides、build core Op、submit_core_op、submit_with_trace 和 core queue 的顺序">
  <figcaption>图 7. turn/start 边界图按源码顺序压缩成七步：load_thread -> map v2 input -> apply overrides -> build core Op -> submit_core_op -> submit_with_trace -> core queue。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-sampling-followup-loop.png" alt="一次 turn 的 sampling follow-up loop：RegularTask::run 先发 TurnStarted，再进入 run_turn，run_turn 根据 needs_follow_up 继续 sampling 或返回，外层 task wrapper 最后发 TurnComplete">
  <figcaption>图 8. sampling follow-up 图把 run_turn 内部的一次或多次 sampling、tool output、history、needs_follow_up 和外层 task wrapper 发出的 TurnComplete 分开。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-build-prompt-model-view.png" alt="build_prompt 投影模型视图：history ledger、base instructions、ToolRouter specs、personality 和 output_schema 汇入 Prompt，再交给 ModelClientSession::stream">
  <figcaption>图 9. build_prompt 图只保留 history、instructions、tool specs、output schema 和模型流入口。</figcaption>
</figure>
...
这三种视图分开以后，事件回流也不能再被理解成“模型直接更新 UI”。模型 stream 只产生 provider events；core 把它们归一成 `EventMsg`；App Server 再把其中一部分翻译成 notification 或 request。图 10 只画回流，不再重复 prompt 构造，是为了让读者把模型视图和 UI 事件路由分开检查。
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-event-return-path.png" alt="事件回流路径：EventMsg 到 conversation.next_event，再到 apply_bespoke_event_handling，分成 ServerNotification to UI 与 ServerRequest to UI，回答再回到 core policy">
  <figcaption>图 10. 事件回流图只保留 EventMsg -> conversation.next_event -> apply_bespoke_event_handling -> ServerNotification to UI / ServerRequest to UI -> answer returns to core policy。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v4-turn-runtime-route-map.png" alt="Codex turn runtime 最终诊断图：UI、protocol、core queue、model view 四条边界与 evidence ledger 帮助定位 run_turn、事件回流和 history ownership">
  <figcaption>图 11. 最终诊断图不再重复完整闭环，而是把四条边界和 evidence ledger 放在同一张 route map 上：先定位边界，再回到精确源码证据。</figcaption>
</figure>
