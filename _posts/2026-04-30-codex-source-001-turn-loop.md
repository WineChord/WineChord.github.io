...
第一条是 client 线：用户输入先被 TUI 组织成 `UserInput`，而不是马上变成模型消息。第二条是 protocol 线：`turn/start` 把 UI 意图、工作目录、审批策略、沙箱、模型和协作模式一起带进 App Server。第三条是 core 线：`Session` 接到 `Op` 后启动 `RegularTask`，再由 `run_turn` 在模型流、工具调用、history 和 follow-up 之间循环。

后面所有函数名都服务于这三条线。只要能回答“这段代码是在 client 侧保存交互状态、在 App Server 做协议转换，还是在 core 里推进 agent loop”，源码结构就不会乱。
...
读 Codex turn path 时，可以用这几条规则快速定位代码职责：

| 问题 | 优先看哪里 | 说明 |
| --- | --- | --- |
| 用户输入怎么被表示？ | `UserInput` / TUI `ChatWidget` | UI 保留意图，不直接拼 prompt。 |
| 一次 turn 的运行配置从哪里来？ | `turn/start` params | cwd、approval、sandbox、model 等和文本一起进入 core。 |
| 模型为什么会多次请求？ | `RegularTask::run` / `run_turn` | 工具输出会进入 follow-up sampling，直到 turn 完成。 |
| UI 为什么能和 core 解耦？ | `EventMsg` / app-server notification | core 只发事件，TUI 自己消费和渲染。 |
