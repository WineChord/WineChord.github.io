...
上下文工程最容易被读成“prompt 模板很长”。这篇要反过来读：先看哪些信息是稳定基线，再看哪些信息会随 turn 变化，最后看长会话如何压缩和恢复。

具体可以分成三个问题：

1. 初始上下文从哪里来：`base_instructions`、AGENTS.md、environment、skills、plugins、apps、memory。
2. 后续上下文怎么变化：`reference_context_item` 只追踪部分 settings diff，而不是完整 prompt diff。
3. 历史太长怎么办：compaction 写入 `replacement_history`，rollout JSONL 负责生产恢复，rollout trace 只做诊断。

只要把这三个问题分开，就不会把 memory、skills、compaction、rollout trace 都混成“给模型多塞一些文本”。
...
![Codex context diff 范围：initial context 很大，后续 tracked settings diff 目前只覆盖一部分字段，源码里也保留 TODO](/assets/images/posts/2026-04-30-codex-source/context-diff-scope.png)
...
读 Codex 上下文相关代码时，先问下面几件事：

| 机制 | 应该怎么理解 | 不要误读成 |
| --- | --- | --- |
| `base_instructions` | 稳定系统基线 | 所有上下文都塞进 system prompt。 |
| AGENTS.md / env | model-visible contextual fragments | 代码级安全边界。 |
| `reference_context_item` | tracked settings diff 的 baseline | 完整动态上下文 diff。 |
| compaction | 用 `replacement_history` 安装新 checkpoint | 简单删除旧消息。 |
| memory write | 隔离后台 agent 和受控 workspace diff | 当前主 agent 随手写长期记忆。 |
| rollout trace | opt-in 本地诊断图谱 | 生产恢复数据源。 |
