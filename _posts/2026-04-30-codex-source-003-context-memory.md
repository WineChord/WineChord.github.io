...
*图 1. 左侧是初始上下文来源，中间是 initial context 和 `ContextManager`，右侧是 compaction 与 rollout。`reference_context_item` 是 tracked settings diff 的 baseline，不是所有动态上下文的万能快照。*
...
但这里必须讲准确：[`build_settings_update_items`][context-updates] 只覆盖 tracked context fields，源码注释也直接留下 TODO，说它还没有覆盖 `build_initial_context` 发出的每一种 model-visible item。因此更准确的说法是：
...
*图 2. 这是本篇最容易误读的地方。tracked settings diff 目前覆盖模型、权限、environment 等字段；它不是对所有 initial context 的完整语义 diff。*
