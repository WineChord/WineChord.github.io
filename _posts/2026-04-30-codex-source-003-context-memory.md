...
header:
  og_image: https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-cover.png
  teaser: https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-cover.png
  teaser_alt: "Codex 上下文系统总览：base instructions、AGENTS.md、skills、apps、memory、compaction 和 rollout 的恢复边界"
categories:
  - LLM
  - Agent
tags:
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-cover.png" alt="Codex 上下文系统总览：base instructions、AGENTS.md、skills、apps、memory summary 汇入 ContextManager，再输出 model view、compaction 和 rollout JSONL">
  <figcaption>图 1. Codex 的 prompt 是一套可追踪、可压缩、可恢复的状态系统；左侧来源、中间 ContextManager、右侧模型视图与恢复日志分别有不同 owner。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-system-overview.png" alt="Codex 上下文工程总览：SessionMeta、TurnContext、AGENTS.md、skills、apps 和 memory 进入 build_initial_context，再产生 developer items、user context items、tracked settings diff、CompactedItem 和 rollout">
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-responses-contract.png" alt="Responses API 与 Codex 请求形状对照：instructions、tools 和 input items 分别映射到 Codex 的 base_instructions、tool specs 和 ResponseItem::Message">
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-model-view-layers.png" alt="Codex 模型视图分层：instructions、developer items、user context、history messages 和 compact summary 汇入 Responses request">
  <figcaption>图 4. 模型看到的是投影后的 Responses request；UI、rollout storage 和 trace graph 可以保留更多或不同形态的证据。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-base-instructions-source.png" alt="base instructions 来源优先级：config override、rollout SessionMeta、model defaults 汇入 session 的 base_instructions">
  <figcaption>图 5. `base_instructions` 的优先级保护的是 resume/fork 的稳定起点，而不是让当前模型默认值覆盖旧会话。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-initial-injection.png" alt="Codex 首个真实 turn 注入：developer bundle、contextual user bundle 和 reference baseline 写入 history items 与 TurnContextItem">
  <figcaption>图 6. 首个真实 turn 同时建立模型可见上下文和后续 diff 的 reference baseline。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-diff-scope.png" alt="tracked settings diff 范围：environment、permissions、collaboration mode、realtime、personality、model 可 diff，apps details、skills body 和部分 initial items 仍需 full injection 或 replay">
  <figcaption>图 7. tracked diff 的价值是降低重复注入；它不是整个上下文系统的身份定义。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-agents-boundary.png" alt="AGENTS.md 边界：project root、subdir、cwd 的 AGENTS.md 串接为 user fragments，approval policy 和 filesystem policy 才是执行边界">
  <figcaption>图 8. AGENTS.md 是模型可见的用户上下文；真正约束工具 action 的是 approval、sandbox、filesystem policy 和工具实现。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-capabilities-lazy.png" alt="skills、plugins、apps 能力摘要：metadata 先进入 instructions，需要时再加载 skill body、tool_search 或 MCP 能力">
  <figcaption>图 9. 能力摘要保护的是上下文预算和能力发现路径；它不是工具执行结果，也不是项目规则。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-memory-read-path.png" alt="memory read path：memory_summary.md 经 token limit 截断后进入 developer instructions，空文件不产生 model-visible item">
  <figcaption>图 10. memory read path 的边界是文件来源、截断策略和 developer-role 入口；开启 memory 不等于每轮都有额外上下文。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-read-write.png" alt="memory read/write 双通道：read side 从 summary 进入 prompt，startup write side 执行 phase 1 filter 和 phase 2 consolidation">
  <figcaption>图 11. read path 影响当前 prompt；write path 是异步整理长期材料，失败或跳过不会直接改变当前模型视图。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-compaction-checkpoint.png" alt="compaction checkpoint：old history 与 recent users 被压缩为 CompactedItem.message，并携带 replacement_history 作为 resume 的恢复基线">
  <figcaption>图 12. compaction 的产物不是普通摘要，而是带 replacement history 语义的 checkpoint。</figcaption>
</figure>
...
#### reverse scan 找 checkpoint，forward replay 保 tail

<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-rollout-recovery.png" alt="rollout recovery：newest-first reverse scan 找到 replacement_history、previous turn settings 和 reference_context_item，再 forward replay surviving tail">
  <figcaption>图 13. Resume 从最新可存活 checkpoint 逆向定位，再正向回放 checkpoint 之后的尾部。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-trace-diagnostic.png" alt="rollout trace 诊断路径：CODEX_ROLLOUT_TRACE_ROOT 启用本地 bundle，raw events 和 payloads 经 replay_bundle 还原 semantic graph，但不作为 resume state">
  <figcaption>图 14. Trace 用来解释运行时证据；生产 resume 仍然走 rollout reconstruction。</figcaption>
</figure>
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260614v3-context-memory-final-rules.png" alt="Codex 上下文系统最终规则：separate views、pin sources、diff tracked state、compact with replacement history、trace only for diagnosis">
  <figcaption>图 15. 一个成熟 coding agent 的 prompt 不只是输入文本，而是可追踪、可压缩、可恢复的状态系统。</figcaption>
</figure>
