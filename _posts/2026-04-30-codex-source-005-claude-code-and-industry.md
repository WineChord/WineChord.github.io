...
mathjax: true
header:
  og_image: https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v1-industry-comparison-cover.png
  teaser: https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v1-industry-comparison-cover.png
  teaser_alt: "Claude Code、Codex 和 OpenClaw 的架构压力对照"
---
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v1-industry-comparison-cover.png" alt="三种 coding agent 架构压力：Claude Code sample、Codex 和 OpenClaw 分别对应 integrated CLI runtime、agent harness 和 gateway-first control plane">
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v1-comparison.png" alt="Codex、Claude Code 和 OpenClaw 的 owner 对照：Claude Code 是集成式 CLI runtime，Codex 是协议化 agent harness，OpenClaw 是 gateway-first control plane">
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v1-claude-cli-runtime-loop.png" alt="Claude Code 对照仓库的集成式 CLI runtime：CLI bootstrap 进入 QueryEngine 和 query loop，再连接 UI state、tools、MCP、permissions、memory 和 skills">
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v1-codex-harness-boundaries.png" alt="Codex harness 边界：JS launcher、Rust CLI、App Server、core session、tools runtime、context ledger、rollout JSONL 和 trace 分层">
...
这也解释了为什么图 4 把 context ledger、rollout JSONL 和 trace 分开画：context ledger 参与下一轮模型视图，rollout JSONL 参与生产恢复，trace 只参与诊断解释。三者都像“记录”，但 owner 和生命周期不同；读源码时如果只按“日志”归类，会误读 Codex 为何同时保留这些结构。

这个 owner 思路会自然过渡到 OpenClaw。OpenClaw 不是从 Codex 的 thread/turn 和 rollout 恢复出发，而是先把 Gateway、operator、node role、sandbox 和 ACP runtime 画成控制面。它解决的是另一组拆分压力。
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v1-openclaw-gateway-control-plane.png" alt="OpenClaw gateway-first control plane：operator 与 TUI client 连接 Gateway，Gateway 路由到 node role 的 camera、screen、system.run 能力以及 ACP runtime">
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v1-industry-axis.png" alt="Coding agent 演进轴：从补全聊天到 ReAct、ACI，再到可约束、可恢复、可审计的 agent harness">
...
<figure class="wc-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/codex-source-20260615v2-agent-architecture-scorecard.png" alt="Agent 架构 scorecard：Integrated CLI、Protocol harness、Gateway control 在 latency、multi-surface、permissions、recovery、observability 上的对照">
  <figcaption>图 7. 这张 scorecard 把 latency、multi-surface、permissions、recovery 和 observability 放在同一张图上，用来暴露三种 owner 的强弱压力。</figcaption>
</figure>
